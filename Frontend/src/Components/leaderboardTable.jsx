...
const LeaderboardTable = ({data,label,board,userLeagues}) => {
  function highlight(index){
    if(index<3){
      return 'bg-green-600 w-9 h-7 p-1 rounded-md text-center ml-12';
    }else if(index<data.length-3){
      return 'w-9 h-7 p-1 rounded-md text-center ml-12'
    }
    return 'bg-yellow-600 w-9 h-7 p-1 rounded-md text-center ml-12';
  }
